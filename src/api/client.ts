...
class ApiClient {
  ...
  getTradeStats(): Promise<TradeStats> {
    return this.get("/trades/stats/summary");
  }

  openTrade(
    req: TradeOpenRequest
  ): Promise<TradeRecord> {
    return this.post("/trades/open", req);
  }
  ...
  runStrategy(
    ...
    return this.post("/strategy/run", req);
  }

  getStrategyStatus(): Promise<Record<string, unknown>> {
